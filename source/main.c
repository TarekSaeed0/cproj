...
int main(int argc, char **argv) {
	...
	else project_name = project_path;
	...
	assert(fgets(cmake_version_command_buffer, sizeof cmake_version_command_buffer, cmake_version_command_file) != NULL);
	...
	char *project_cmakelists_txt_path = malloc(strlen(project_path) + strlen("/CMakeLists.txt") + 1);
	assert(project_cmakelists_txt_path != NULL);
	strcat(strcpy(project_cmakelists_txt_path, project_path), "/CMakeLists.txt");
	...
	assert(fprintf(project_cmakelists_txt_file,
		...
		"    -std=c17 -O2 -Werror -Wall -Wextra -pedantic -Wfloat-equal -Wundef\n"
		...
		")\n",
	cmake_version, project_name) > 0);
	assert(fclose(project_cmakelists_txt_file) != EOF);

	char *project_compile_commands_json_path = malloc(strlen(project_path) + strlen("/compile_commands.json") + 1);
	assert(project_compile_commands_json_path != NULL);
	strcat(strcpy(project_compile_commands_json_path, project_path), "/compile_commands.json");
	assert(symlink("build/compile_commands.json", project_compile_commands_json_path) != -1);
	free(project_compile_commands_json_path);

	char *project_source_path = malloc(strlen(project_path) + strlen("/source") + 1);
	...
	char *project_source_main_c_path = malloc(strlen(project_source_path) + strlen("/main.c") + 1);
	...
	assert(fprintf(project_source_main_c_file,
		"#include <stdio.h>\n"
		"\n"
		"int main(void) {\n"
		"	printf(\"Hello, World!\\n\");\n"
	...
	char *project_resource_path = malloc(strlen(project_path) + strlen("/resource") + 1);
	...
	char *project_header_path = malloc(strlen(project_path) + strlen("/header") + 1);
	...
	char *project_library_path = malloc(strlen(project_path) + strlen("/library") + 1);
	...
	char *project_build_path = malloc(strlen(project_path) + strlen("/build") + 1);
	assert(project_build_path != NULL);
	strcat(strcpy(project_build_path, project_path), "/build");

	char *cmake_build_command = malloc(strlen("cmake -S ") + strlen(project_path) + strlen(" -B ") + strlen(project_build_path) + strlen(" > /dev/null") + 1);
	assert(cmake_build_command != NULL);
	strcat(strcat(strcat(strcat(strcpy(cmake_build_command, "cmake -S "), project_path), " -B "), project_build_path), " > /dev/null");
